...
impl BufferGroup {
	pub fn new(id: u32, bind_group: Option<BindGroup>) -> Self {
		BufferGroup {
			id,
			binding_type: match (&bind_group) {
				Some(_) => BufferBindingType::BindGroup,
				None => BufferBindingType::Vertex,
			},
			bind_group,
			buffers: HashMap::new(),
		}
...
impl Shader {
	...
	fn update_buffer(&self, queue: &wgpu::Queue, label: String, data: &[u8], offset: u64) {
		let buffer = self
			.buffers
			.iter()
			.find_map(|bg| bg.buffers.get(&label))
...
pub fn create_graphics(window: SharedPtr<winit::window::Window>) -> GraphicsHandle {
...
pub fn create_shader_pipeline(
	...
	shader_buffers.push(
		BufferGroup::new(0, None).with_buffer("Canvas Vertex Buffer".to_string(), canvas_buffer),
	);
...
impl GraphicsHandle {
...

		{
			...
			for buffer_group in &shader.buffers {
				let _ = match buffer_group.binding_type {
					BufferBindingType::BindGroup => render_pass.set_bind_group(
						buffer_group.id.clone(),
						&buffer_group.bind_group,
						&[],
					),
	...
	pub fn spawn_render_loop(self) {
		loop {
			...
			shad.update_buffer(
				&self.queue,
				"Canvas Vertex Buffer".to_string(),
				bytemuck::cast_slice(&new_vert),
				0,
			);
