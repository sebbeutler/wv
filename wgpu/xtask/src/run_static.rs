...
pub(crate) fn run_static(shell: &Shell, mut args: Arguments) -> anyhow::Result<()> {
    ...
    xshell::cmd!(
        shell,
        "wasm-bindgen target/wasm32-unknown-unknown/{output_dir}/wgpu-examples.wasm --target web --typescript --out-dir target/generated --out-name webgpu"
    )
    .quiet()
    .run()
    ...
    xshell::cmd!(
        shell,
        "wasm-bindgen target/wasm32-unknown-unknown/{output_dir}/wgpu-examples.wasm --target web --typescript --out-dir target/generated --out-name webgl2"
    )
    .quiet()
    .run()
    .context("Failed to run wasm-bindgen")?;
