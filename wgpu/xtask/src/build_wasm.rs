...
pub(crate) fn build_wasm(shell: &Shell, mut args: Arguments) -> anyhow::Result<()> {
    ...
    xshell::cmd!(
        shell,
        "wasm-bindgen target/wasm32-unknown-unknown/{target}/wgpu-examples.wasm --target web --typescript --out-dir {output_dir} --out-name webgpu"
    )
    .quiet()
    .run()
    ...
    xshell::cmd!(
        shell,
        "wasm-bindgen target/wasm32-unknown-unknown/{target}/wgpu-examples.wasm --target web --typescript --out-dir {output_dir} --out-name webgl2"
    )
    .quiet()
    .run()
    .context("Failed to run wasm-bindgen")?;
